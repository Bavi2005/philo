...
test_4 ()
{
		# #test 1 / 1
		pkill -x "$2" > /dev/null
		output=$("$1" 3 310 200 100 >> log ; sleep 1 ; pkill -x "$2" ; cat log | grep died -m 1 | awk '{print $NF}')
		if [ "$output" == "died" ]
				then
						echo -en "${SP}${Green}OK${White}"
		else
		...
		# #test 2 / 1
		output=$("$1" 3 800 400 400 > log2 ; sleep 0.9 ; pkill -x "$2" ; cat log2 | grep died -m 1| awk '{print $NF}')
		if [ "$output" == "died" ]
				then
						echo -en "${SP}${Green}OK${White}"
		else
		...
		output=$("$1" 10 800 400 200 > log3 ; sleep 0.9 ; pkill -x "$2" ; cat log3 | grep died -m 1| awk '{print $NF}')
		if [ "$output" == "died" ]
				then
						echo -en "${SP}${Green}OK${White}"
		else
